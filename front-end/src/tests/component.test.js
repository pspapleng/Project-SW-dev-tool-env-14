...
describe("get correct name or province for search", () => {
  const mockGetAllServiceCenter = [
    {
  ...
  it("get correct name service center by API mockGetAllServiceCenter", async () => {
    const wrapper = shallow(<ServiceCenter />);
    wrapper.find("#search-bar");
    const check = jest.fn();
    const mockFn = jest.fn();
    mockFn.mockReturnValue(check);
    check.mockResolvedValue(mockGetAllServiceCenter[0].name);
    await expect(check()).resolves.toEqual("Center for Psychological Wellness");
  });
  it("get correct province service center by API mockGetAllServiceCenter", async () => {
    const wrapper = shallow(<ServiceCenter />);
    wrapper.find("#search-bar");
    const check = jest.fn();
    const mockFn = jest.fn();
    mockFn.mockReturnValue(check);
    check.mockResolvedValue(mockGetAllServiceCenter[0].province);
    await expect(check()).resolves.toEqual("เชียงใหม่");
  });
  it("get correct service center by API mockGetAllServiceCenter", async () => {
    const wrapper = shallow(<ServiceCard data={mockGetAllServiceCenter} />);
    //remove 1 service (find 19 service from 20 service)
    const i = mockGetAllServiceCenter.pop();
    wrapper.setProps({ mockGetAllServiceCenter });
    expect(wrapper.find("#name-card")).toHaveLength(19);
    //add 1 service (find 20 service from 20 service)
    mockGetAllServiceCenter.push(i);
    wrapper.setProps({ mockGetAllServiceCenter });
